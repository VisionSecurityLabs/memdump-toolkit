...
def _known_good_option(f) -> Any:
    ...
    f = click.option("--known-good", "known_good_path", default=None,
                     ...
                     help="File of known-good SHA-256 hashes (NSRL/plain text)")(f)
    return f
...
@_common_options
def extract(dump, out_dir, verbose, yara_dir):
...
@click.argument("dump", type=click.Path(exists=True))
@_common_options
def detect(dump, out_dir, verbose, yara_dir):
...
@click.argument("dump", type=click.Path(exists=True))
@_common_options
def go_scan(dump, out_dir, verbose, yara_dir):
...
@_common_options
def go_info(binary, out_dir, verbose, yara_dir):
...
@_common_options
def config(input_file, is_dump, out_dir, verbose, yara_dir):
...
@click.argument("dump", type=click.Path(exists=True))
@_common_options
def dotnet_scan(dump, out_dir, verbose, yara_dir):
...
@click.argument("binary", type=click.Path(exists=True))
@_common_options
def inspect(binary, out_dir, verbose, yara_dir):
...
@_common_options
@_known_good_option
def binary_scan(dump, out_dir, verbose, yara_dir, known_good_path):
    """Universal binary analysis (score all DLLs, language-agnostic)."""
    from memdump_toolkit.analyze_binary import load_known_good_hashes, run
    known_good = None
    if known_good_path:
        known_good = load_known_good_hashes(known_good_path)
        click.echo(f"Loaded {len(known_good)} known-good hashes from {known_good_path}")
    run(dump, out_dir=out_dir, verbose=verbose, yara_rules_dir=yara_dir,
...
@click.argument("dump", type=click.Path(exists=True))
@_common_options
def c2_hunt(dump, out_dir, verbose, yara_dir):
...
@_common_options
@_known_good_option
def full(dump, out_dir, verbose, yara_dir, known_good_path):
    """Run the complete 5-step analysis pipeline."""
    known_good = None
    if known_good_path:
        from memdump_toolkit.analyze_binary import load_known_good_hashes
        known_good = load_known_good_hashes(known_good_path)
        click.echo(f"Loaded {len(known_good)} known-good hashes from {known_good_path}")
